import React, { SVGProps } from 'react';
interface ThemeProps extends SVGProps<SVGSVGElement> {
	color: string;
	width?: string;
	height?: string;
}
const Theme: React.FC<ThemeProps> = ({ width = 26, height = 26, color, ...rest }) => {
	return (
		<svg
			width={width}
			height={height}
			viewBox="0 0 26 26"
			fill="none"
			xmlns="http://www.w3.org/2000/svg"
			{...rest}

		//   onClick={onClick}
		>
			<path
				d="M13 2C11.5555 2 10.1251 2.28452 8.79048 2.83733C7.4559 3.39013 6.24327 4.20038 5.22182 5.22183C4.20038 6.24327 3.39013 7.4559 2.83732 8.79048C2.28452 10.1251 2 11.5555 2 13C2 14.4445 2.28452 15.8749 2.83733 17.2095C3.39013 18.5441 4.20038 19.7567 5.22183 20.7782C6.24327 21.7996 7.4559 22.6099 8.79048 23.1627C10.1251 23.7155 11.5555 24 13 24L13 13L13 2Z"
				fill={color}
			/>
			<path
				fillRule="evenodd"
				clipRule="evenodd"
				d="M25.5 13C25.5 19.9035 19.9035 25.5 13 25.5C6.09649 25.5 0.5 19.9035 0.5 13C0.5 6.09649 6.09649 0.5 13 0.5C19.9035 0.5 25.5 6.09649 25.5 13ZM13 24C19.0751 24 24 19.0751 24 13C24 6.92492 19.0751 2 13 2C6.92492 2 2 6.92492 2 13C2 19.0751 6.92492 24 13 24Z"
				fill={color}
			/>
			<path
				d="M13 18.75C16.1756 18.75 18.75 16.1756 18.75 13C18.75 9.82439 16.1756 7.25 13 7.25C9.82439 7.25 7.25 9.82439 7.25 13C7.25 16.1756 9.82439 18.75 13 18.75Z"
				fill="white"
				stroke="black"
				strokeWidth="1.5"
				strokeLinecap="round"
				strokeLinejoin="round"
			/>
			<path
				d="M13 18C13.6566 18 14.3068 17.8707 14.9134 17.6194C15.52 17.3681 16.0712 16.9998 16.5355 16.5355C16.9998 16.0712 17.3681 15.52 17.6194 14.9134C17.8707 14.3068 18 13.6566 18 13C18 12.3434 17.8707 11.6932 17.6194 11.0866C17.3681 10.48 16.9998 9.92876 16.5355 9.46447C16.0712 9.00017 15.52 8.63188 14.9134 8.3806C14.3068 8.12933 13.6566 8 13 8L13 13L13 18Z"
				fill={color}
			/>
		</svg>
	)
}
export default Theme;